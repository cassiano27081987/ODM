...
def constructConjunctiveGraph():
    ...
    for job in JOBSLIST:
        ...
        for seq1 in job.jobSequence:
            ...
            for seq2 in job.jobSequence:
                if seq1 >= seq2 and seq1 ==seq2:
                    conjunctiveGraph[seq1][seq2] = NODESLIST[seq2+1].duration
...
def resetNodes():
    ...
    for k in range(K):
        ...
        print("sinkDepenetes são: ", Sink.dependents[k])
############################## END OF CYCLE --- END
...
def makeGanttChart(bestSoFarAnt):
    ...
    for job in JOBSLIST: #I believe this could stay as JOBSLIST since we pass in only the node.num into the bestSoFarNODESLIST
        for node in job.Nodes:
            if node.duration> 0 and node.duration != 0:
                ...
                print("\n")
                s = str(datetime.datetime.strptime('2022-09-16 00:00:00', "%Y-%m-%d %H:%M:%S"))
                ...
                if s!= 0 and d != 0:
                    df.append(dict(Task=str(node.machine), Start=str(s), Finish=str(d), Resource=str(job.num), OP=str(param_seq.numJobs)))
    ...
    fig = ff.create_gantt(df, colors = colors, index_col='Resource', title = "ANT COLONY OPTIMIZATION - OPTIMUS SOLUTIONS",show_colorbar=True, group_tasks=True,showgrid_x =True)

...
def ler_dados(p):
    ...
    #table_processamento = pd.pivot_table(df_completo, values =['Ord Prod'],  columns = ['Grp Maq'],aggfunc=np.sum,fill_value=0)
    ...
    #table_sequencia= pd.pivot_table(df_completo, values=['Oper'], columns=['Grp Maq','Oper'],index=['Ord Prod'],fill_value=0)

    colunas_selecionadas2 = ['Ord Prod','Oper','Grp Maq','Tmp Máq']

    table_sequencia= df_completo[colunas_selecionadas2]

    table_sequencia.sort_values(by=['Ord Prod'])

    #print("Tabela da sequencia")
    #print(table_sequencia)
    if p ==0 :
        return df_data_op
    if p ==1 :

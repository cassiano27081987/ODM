...
#!!!!!!!!!!!!!!!!!!!!!!!!!!! DATA ENTRY --- START !!!!!!!!!!!!!!!!!!!!!!!!!!!#
    ...
    #numJobs = datas['Ord Prod'].values
    #numMachines = grupo_maq['Grp Maq'].values[:-1]
    #numNodes =  len(numJobs)*len(numMachines) + 2
    numNodes = numJobs*numMachines + 2
    
    #max_oper = np.max(seq_process['Oper'].values)
...
def makeGanttChart(bestSoFarAnt):
    ...
    for job in JOBSLIST: #I believe this could stay as JOBSLIST since we pass in only the node.num into the bestSoFarNODESLIST
        for node in job.Nodes:
            print(node.num)
            print(job.num)
            s = str(datetime.datetime.strptime('2020-11-16 00:00:00', "%Y-%m-%d %H:%M:%S") + datetime.timedelta(days=bestSoFarNODESLIST[node.num].startTime))
